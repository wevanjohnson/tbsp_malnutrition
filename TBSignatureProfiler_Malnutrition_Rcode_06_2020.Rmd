...
TBsignatures$"BATF" <- "BATF"
TBsignatures$"NPC2" <- "NPC2"
siglist_mal <- names(TBsignatures)

SummarizedExperiment::colData(ssgsea_res)$`BATF` <- assay(indata, "log_cpm")["BATF", ]
SummarizedExperiment::colData(ssgsea_res)$`NPC2` <- assay(indata, "log_cpm")["NPC2", ]

SummarizedExperiment::colData(gsva_res)$`BATF` <- assay(indata, "log_cpm")["BATF", ]
SummarizedExperiment::colData(gsva_res)$`NPC2` <- assay(indata, "log_cpm")["NPC2", ]

SummarizedExperiment::colData(plage_res)$`BATF` <- assay(indata, "log_cpm")["BATF", ]
...
```{r}
  ...
  for (i in signatureColNames) {
    ...
    out.ssgsea[which.sig, 8] <- round(conf[2], 4)
  }
