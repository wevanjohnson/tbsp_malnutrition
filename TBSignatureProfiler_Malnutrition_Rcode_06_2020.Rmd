...
```{r}
  ...
  for (i in signatureColNames) {
    ...
    out.ssgsea[which.sig, 4] <- conf[1]
    out.ssgsea[which.sig, 6] <- conf[2]
  }
  
  colnames(out.ssgsea)[c(4, 6)] <- c("LowerCI", "UpperCI")
...
```{r}
  ...
  for (i in signatureColNames) {
    ...
    roc1 <- suppressMessages(pROC::roc(predictor = score,
                                       response = annotationData))
    conf <- pROC::ci.auc(roc1, method = "delong", progress = "none")
    out.gsva[which.sig, 4] <- conf[1]
    out.gsva[which.sig, 6] <- conf[2]
  }
  
  colnames(out.gsva)[c(4, 6)] <- c("LowerCI", "UpperCI")
  datatable(out.gsva)
```
...
```{r}
  ...
  for (i in signatureColNames) {
    ...
    roc1 <- suppressMessages(pROC::roc(predictor = score,
                                       response = annotationData))
    conf <- pROC::ci.auc(roc1, method = "delong", progress = "none")
    out.plage[which.sig, 4] <- conf[1]
    out.plage[which.sig, 6] <- conf[2]
  }
  
  colnames(out.plage)[c(4, 6)] <- c("LowerCI", "UpperCI")
